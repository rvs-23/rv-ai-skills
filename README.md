...
*Note on Terminology*: **Codex** refers to the model/agent behavior, while the **Terminal** is the execution environment. This Hub manages the behavior across environments.
...
### 3. Codex & Other Agents
Inject skills as rules or system prompts:
```bash
# For Cursor rules
...
Importexpertise from major vendors by linking from the `.vendor/` directory.
