...
1. **Single source of truth**: `_registry.json` defines every skill, its vendor, enabled status, and entry-point resolution strategy. The loader reads the registry — not the filesystem — to decide what to sync.
...
├── _registry.json               Single source of truth: all skills, vendors, metadata.
...
| `_registry.json` | Yes | Skill index and vendor configuration |
...
```bash
# Inject a core skill
uv run adapters/skill_loader.py core/writing_good_readme.md claude /path/to/project

# Inject an external skill (three equivalent formats)
uv run adapters/skill_loader.py external/anthropic/pdf claude /path/to/project
uv run adapters/skill_loader.py anthropic:pdf claude /path/to/project

# Inject with compact mode (smaller context footprint)
uv run adapters/skill_loader.py anthropic:pdf claude /path/to/project --compact
```
...
uv run adapters/skill_loader.py --sync-all --include-external
...
```bash
uv run adapters/skill_loader.py core/writing_good_readme.md codex /path/to/project
uv run adapters/skill_loader.py anthropic:pdf codex /path/to/project
```

**Updating a changed skill**:

```bash
uv run adapters/skill_loader.py --update-skill writing_good_readme
```
...
```bash
uv run adapters/skill_loader.py core/writing_good_readme.md gemini /path/to/project
uv run adapters/skill_loader.py anthropic:pdf gemini /path/to/project
```
...
```bash
uv run adapters/skill_loader.py core/writing_good_readme.md cursor /path/to/project
uv run adapters/skill_loader.py anthropic:pdf cursor /path/to/project
```
...
uv run adapters/skill_loader.py core/writing_good_readme.md openai .

# Compact mode for token-limited contexts
uv run adapters/skill_loader.py anthropic:pdf openai . --compact
```
...
```bash
uv run adapters/skill_loader.py <skill_ref> <target> <project_path> [--compact]
```
...
uv run adapters/skill_loader.py --validate

# Validate a single file
uv run adapters/skill_loader.py --validate core/writing_good_readme.md
```
...
```bash
uv run adapters/skill_loader.py --update-skill writing_good_readme
uv run adapters/skill_loader.py --update-skill anthropic:pdf
```
...
```bash
# Validate
uv run adapters/skill_loader.py --validate core/your_skill_name.md

# Register in `_registry.json` (add an entry under `skills.core`)
...
uv run adapters/skill_loader.py core/your_skill_name.md claude /path/to/project
```
...
2. Register it in `_registry.json` under the vendor's skill list:
...
This reads `_registry.json` and creates any missing symlinks, removes disabled ones.
...
4. **How do I update a skill I already synced?**
   `uv run adapters/skill_loader.py --update-skill <name>`. This removes the old block and re-injects the current version.
...
8. **How do I add skills to all my projects at once?**
   ...
   for project in ~/projects/*/; do
       uv run adapters/skill_loader.py core/your-skill.md claude "$project"
   done
   ```
